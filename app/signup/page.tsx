      ...
      <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col">
        ...
        <div className="mb-8">
          ...
          <p className="text-charcoal-600">
            {step === 1 ? "Start your 14-day free trial today" : "Select the plan that works best for your restaurant"}
          </p>
        </div>
        ...
        <div className="flex items-center mb-8">
          <div className="flex items-center">
            <div
              className={`flex h-8 w-8 items-center justify-center rounded-full ${step >= 1 ? "bg-saffron-500 text-white" : "bg-cream-200 text-charcoal-500"}`}
            >
              {step > 1 ? <Check className="h-4 w-4" /> : "1"}
            </div>
            <span className={`ml-2 text-sm ${step >= 1 ? "text-burgundy-500 font-medium" : "text-charcoal-500"}`}>
              Account
            </span>
          </div>
          <div className={`h-px w-12 mx-2 ${step >= 2 ? "bg-saffron-500" : "bg-cream-300"}`}></div>
          <div className="flex items-center">
            <div
              className={`flex h-8 w-8 items-center justify-center rounded-full ${step >= 2 ? "bg-saffron-500 text-white" : "bg-cream-200 text-charcoal-500"}`}
            >
              2
            </div>
            <span className={`ml-2 text-sm ${step >= 2 ? "text-burgundy-500 font-medium" : "text-charcoal-500"}`}>
              Plan
            </span>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 flex-1">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="rounded-xl border-cream-300 focus:border-saffron-500 focus:ring-saffron-500"
              />
              <p className="text-xs text-charcoal-500">Must be at least 8 characters</p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="terms" className="text-sm font-normal cursor-pointer">
                I agree to the{" "}
                <Link href="/terms" className="text-saffron-500 hover:underline">
                  Terms of Service
                </Link>{" "}
                and{" "}
                <Link href="/privacy" className="text-saffron-500 hover:underline">
                  Privacy Policy
                </Link>
              </Label>
            </div>
            ...
            <p className="text-center text-sm text-charcoal-600 mt-8">
              ...
              <Link href="/login" className="text-saffron-500 hover:text-saffron-600 font-medium transition-colors">
                Sign in
              </Link>
            </p>
          </form>
                ...
                <div key={plan.id} className="relative">
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="font-bold text-lg font-playfair text-burgundy-500">{plan.title}</h3>
                        <p className="text-sm text-charcoal-600">{plan.description}</p>
                      </div>
                      <div className="text-right">
                        <span className="text-xl font-bold text-burgundy-500">{plan.price}</span>
                        {plan.price !== "Custom" && <span className="text-sm text-charcoal-500">/month</span>}
                      </div>
                    </div>
                    <div className="space-y-2">
          ...
          <div className="w-full max-w-md">
            {step === 1 ? (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="italic text-charcoal-700 mb-2">
                          "MenuMagic transformed how we present our menu to customers. The QR cards look elegant and
                          match our restaurant's decor perfectly."
                        </p>
                        <p className="font-medium text-burgundy-500">Rajesh Kumar</p>
                        <p className="text-sm text-charcoal-600">Owner, Spice Garden, Mumbai</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="space-y-6 animate-fade-up">
                <h3 className="text-2xl font-bold font-playfair text-burgundy-500">What you'll get:</h3>

                <div className="space-y-4">
                  {[
                    {
                      title: "Beautiful Digital Menu",
                      description: "Create a stunning digital menu that reflects your restaurant's style and brand.",
                      icon: (
                        <svg
                    ...
                    },
                    {
                      ...
                      description: "Receive elegantly designed physical QR code cards for your tables.",
                      icon: (
                        <svg
                    ...
                    },
                    {
                      ...
                      description: "Update your menu items, prices, and descriptions anytime with just a few clicks.",
                      icon: (
                        <svg
                    ...
                    },
                    {
                      ...
                      description: "Try all features risk-free for 14 days with no credit card required.",
                      icon: (
                        <svg
                          className="h-6 w-6 text-saffron-500"
                          viewBox="0 0 24 24"
                    ...
                    <Card key={i} className="border-none shadow-card bg-white/80 backdrop-blur-sm">
                      <CardContent className="p-4">
                        <div className="flex items-start gap-4">
                          <div className="flex-shrink-0">{item.icon}</div>
                          <div>
                            <h4 className="font-medium text-burgundy-500">{item.title}</h4>
